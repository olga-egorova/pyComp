...
class Cantilever():
    ...
    def __init__(self, param, comm):
        ...
        nx = 150                      # used to be 50: ratio 1 to 5
        ny = 20

        Lx = 15.                      # used to be 10. made the beam longer
        Ly = 2.
    ...
    def getDataDof(self):
        ...
        self.tmp = dofListTopSurface
        #choices(dofListTopSurface, k=int(self.numDataPoints))

        self.Qdof = np.zeros(len(self.tmp), dtype = np.int32)

        for i in range(len(self.tmp)):
            self.Qdof[i] = 3 * self.tmp[i] + 2             ## why?
...
np.savetxt("out/obs_coords.csv", np.transpose(myModel.coords)[myModel.tmp,:2], delimiter=",")
