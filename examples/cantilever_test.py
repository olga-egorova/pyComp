...
theta0 = np.arange(-np.pi/6, np.pi/6 + 0.001, np.pi/60)
Nsamples = theta0.shape[0]**3 #50
...
for i in range(Nsamples):
    ...
    F_track[i] = F
    theta_track[i] = baseAngles + theta[i,:]

    if any(np.isin(np.array([i]), np.array([1, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000]))):
        np.savetxt('output_grid.csv', F_track, delimiter=",")
        np.savetxt('theta_grid.csv', theta, delimiter=",")
