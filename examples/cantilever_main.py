...
class Cantilever():
    ...
    def __init__(self, param, comm):
        ...
        self.t = np.asarray([0.2, 0.02, 0.2, 0.02, 0.2]) # thicknesses of layers and interfaces
    ...
    def getDataDof(self):
        ...
        dofListTopSurface = []                # was just top surface
        dofListMiddleSurface = []
        dofListBottomSurface = []

        for i in range(self.nnodes):        # add indeces from different layers
            if(self.coords[2,i] > np.sum(self.t) - 1e-6):
                dofListTopSurface.append(i)
            if(np.sum(self.t[:3]) - 1e-6 <= self.coords[2,i] <= np.sum(self.t[:3]) + 1e-6):
                dofListMiddleSurface.append(i)
            if(np.sum(self.t[:1]) - 1e-6 <= self.coords[2,i] <= np.sum(self.t[:1]) + 1e-6):
                dofListBottomSurface.append(i)

        assert len(dofListTopSurface) > self.numDataPoints, "numDataPoints > than num nodes on top surface"

        seed(124) # So the data points are the same

        tmp_top = choices(dofListTopSurface, k=int(self.numDataPoints/3))
        tmp_middle = choices(dofListMiddleSurface, k=int(self.numDataPoints/3))
        tmp_bottom = choices(dofListBottomSurface, k=int(self.numDataPoints/3))

        tmp = tmp_top + tmp_middle + tmp_bottom
        self.tmp = tmp

        self.Qdof = np.zeros(len(tmp), dtype = np.int32)

        for i in range(len(tmp)):
            self.Qdof[i] = 3 * tmp[i] + 2             ## why?
...
#
#     print(F)
# np.savetxt("LocalCoords.csv", myModel.da.getCoordinatesLocal()[:], delimiter=",")
# np.savetxt("AllCoords.csv", np.transpose(myModel.coords), delimiter=",")
#
# #np.savetxt("obs_coords.csv", np.transpose(myModel.da.getCoordinatesLocal()[myModel.Qdof]).reshape((myModel.dim, myModel.numDataPoints)), delimiter=",")
# print(myModel.Qdof)
# print(myModel.tmp)
#
# print(myModel.da.getCoordinatesLocal()[myModel.Qdof - 2])
# print(myModel.da.getCoordinatesLocal()[myModel.Qdof - 1])
# print(myModel.da.getCoordinatesLocal()[myModel.Qdof])
#
# print(np.transpose(myModel.coords)[myModel.tmp,:])
# np.savetxt("obs_coords.csv", np.transpose(myModel.coords)[myModel.tmp,:], delimiter=",")
