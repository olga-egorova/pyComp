...
class Cantilever():
    ...
    def getDataDof(self):
        ...
        assert len(dofListTopSurface) > (self.numDataPoints/3), "numDataPoints > than num nodes on top surface"

        seed(124) # So the data points are the same

        tmp_top = dofListTopSurface           # choose all points on the grid
        tmp_middle = dofListMiddleSurface
        tmp_bottom = dofListBottomSurface
...
#print(len(myModel.solve(baseAngles, True, iterativeSolver = False)))
np.savetxt("out/sim_coords.csv", np.transpose(myModel.coords)[myModel.tmp,:2], delimiter=",")
